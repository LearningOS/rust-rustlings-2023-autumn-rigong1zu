...
#[cfg(test)]
mod tests {
    use super::Foo; // 引入mod Foo

    #[test]
    fn test_success() {
        let result1 = Foo::my_demo_function(123); // 调用Foo::my_demo_function
        assert_eq!(result1, 123);

        let result2 = Foo::my_demo_function_alias(456); // 调用Foo::my_demo_function_alias
